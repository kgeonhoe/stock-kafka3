...
class WorkerOptimizationTester:
    ...
    def test_single_symbol(self, symbol: str, delay_range: tuple = (0.1, 0.5), period: str = "5d") -> Dict:
        """단일 심볼 테스트 (실제 API 호출)"""
        ...
        try:
            ...
            print(f"   📡 {symbol} API 호출 중... (기간: {period}, 지연: {delay:.2f}초)")
            ...
            print(f"   ✅ {symbol}: {len(hist)}개 레코드 수집 ({time.time() - start_time:.2f}초)")
            return {
                'symbol': symbol,
                'success': True,
                'duration': time.time() - start_time,
            ...
            # API 제한 에러 상세 로깅
            if is_rate_limit:
                print(f"   🚫 {symbol}: API 제한 감지 - {str(e)[:100]}...")
            else:
                print(f"   ❌ {symbol}: 일반 오류 - {str(e)[:100]}...")
    ...
    def quick_test(self, test_symbols: List[str] = None) -> Optional[int]:
        ...
        if test_symbols is None:
            test_symbols = ["AAPL", "MSFT", "GOOGL", "TSLA", "NVDA", "AMZN", "META", "NVDA", "AMD", "INTC"]
        ...
        for workers in test_workers:
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                futures = [
                    executor.submit(self.test_single_symbol, symbol, period="5d") 
                    for symbol in test_symbols
                ]
    ...
    def production_test(self, test_symbols: List[str] = None, period: str = "1y") -> Optional[int]:
        """프로덕션 환경 시뮬레이션 테스트 (1년치 데이터)"""
        if test_symbols is None:
            test_symbols = ["AAPL", "MSFT", "GOOGL", "TSLA", "NVDA"]
        
        print(f"🏭 프로덕션 환경 시뮬레이션 테스트 시작 (기간: {period})")
        print(f"테스트 심볼: {', '.join(test_symbols)}")
        print("⚠️ 주의: 1년치 데이터는 API 제한이 더 심할 수 있습니다!")
        
        test_workers = [1, 2, 3, 5]  # 보수적인 워커 수
        results = {}
        
        for workers in test_workers:
            print(f"\n🔸 워커 {workers}개 테스트... (기간: {period})")
            start_time = time.time()
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                futures = [
                    executor.submit(self.test_single_symbol, symbol, delay_range=(1.0, 2.0), period=period) 
                    for symbol in test_symbols
                ]
            ...
            # 더 긴 쿨다운 (API 제한 방지)
            if workers != test_workers[-1]:
                print(f"   ⏰ 긴 쿨다운 대기... (15초)")
                time.sleep(15)
        
        # 결과 요약
        print(f"\n📋 프로덕션 테스트 결과 (기간: {period}):")
        if results:
            best_workers = max(results.keys(), key=lambda w: results[w]['efficiency_score'])
...
def main():
    ...
    parser.add_argument('--quick', action='store_true', help='빠른 테스트 실행 (5일 데이터)')
    parser.add_argument('--production', action='store_true', help='프로덕션 테스트 실행 (1년 데이터)')
    parser.add_argument('--period', type=str, default='1y', help='데이터 기간 (1y, 5y 등)')
    ...
    try:
        if args.quick:
            ...
            # 프로덕션 테스트 (1년 또는 5년 데이터)
            optimal_workers = tester.production_test(period=args.period)
        else:
            # 전체 테스트
            worker_counts = None
            if args.workers:
