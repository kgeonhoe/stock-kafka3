...
class BulkDataCollector:
    """대량 데이터 수집 및 최적화된 저장 (yfinance 호환)"""
    ...
    def _initialize_database(self):
        ...
        with self._get_write_connection() as conn:
            # 메모리 및 성능 최적화 설정
            conn.execute('SET memory_limit="1GB"')  # 메모리 제한
            conn.execute("SET threads=2")  # 스레드 수
            ...
            self.logger.info("데이터베이스 초기화 완료 (yfinance 호환 + FDR 원본)")
    
    def _get_write_connection(self):
        """쓰기 전용 연결 반환 (락 재시도 포함)"""
        import time
        max_retries = 5
        for attempt in range(max_retries):
            try:
                ...
                if "Resource temporarily unavailable" in str(e) and attempt < max_retries - 1:
                    ...
                    self.logger.warning(f"DuckDB 락 충돌 감지, {wait_time}초 후 재시도 ({attempt + 1}/{max_retries})")
                    time.sleep(wait_time)
                    continue
                else:
                    self.logger.error(f"DuckDB 연결 실패 (시도 {attempt + 1}/{max_retries}): {e}")
                    raise
    ...
    def collect_single_symbol_data(self, symbol: str, 
        ...
        Returns:
            {'yfinance_compat': df, 'raw_data': df} 또는 None
        """
        try:
            if end_date is None:
            ...
            if all(col in df.columns for col in ['raw_open', 'raw_high', 'raw_low', 'raw_close', 'adj_close']):
                ...
                # 원본 데이터의 날짜도 동일하게 처리
                raw_df = raw_df.copy()
                # 안전한 날짜 처리: 현재 데이터 타입에 따라 다르게 처리
                try:
                    if pd.api.types.is_datetime64_any_dtype(raw_df['date']):
                        # datetime64 타입인 경우
                        raw_df['date'] = raw_df['date'].dt.date
                    elif raw_df['date'].dtype == 'object':
                        # 문자열이나 기타 object 타입인 경우
                        if isinstance(raw_df['date'].iloc[0], str):
                            # 문자열인 경우 먼저 datetime으로 변환
                            raw_df['date'] = pd.to_datetime(raw_df['date']).dt.date
                        else:
                            # 이미 date 객체인 경우 그대로 유지
                            pass
                    # 이미 date 타입이면 그대로 유지
                except Exception as e:
                    self.logger.warning(f"날짜 변환 오류 (raw_df): {e}, 원본 그대로 사용")
                    # 오류 발생시 원본 데이터 그대로 사용
            else:
                raw_df = None
            
            # 데이터 타입 최적화 (복사본 생성으로 SettingWithCopyWarning 방지)
            yfinance_compat_df = yfinance_compat_df.copy()
            # 안전한 날짜 처리: 현재 데이터 타입에 따라 다르게 처리
            try:
                if pd.api.types.is_datetime64_any_dtype(yfinance_compat_df['date']):
                    # datetime64 타입인 경우
                    yfinance_compat_df['date'] = yfinance_compat_df['date'].dt.date
                elif yfinance_compat_df['date'].dtype == 'object':
                    # 문자열이나 기타 object 타입인 경우
                    if isinstance(yfinance_compat_df['date'].iloc[0], str):
                        # 문자열인 경우 먼저 datetime으로 변환
                ...
                self.logger.warning(f"날짜 변환 오류 (yfinance_compat_df): {e}, 원본 그대로 사용")
                # 오류 발생시 원본 데이터 그대로 사용
                    ...
                    try:
                        with self._get_write_connection() as conn:
                            ...
                            if yfinance_dfs:
                                ...
                                # 날짜 컬럼 타입 확인 및 변환
                                self.logger.info(f"날짜 컬럼 타입: {combined_yfinance_df['date'].dtype}")
                                self.logger.info(f"날짜 샘플: {combined_yfinance_df['date'].head()}")
                                
                                # 날짜를 문자열로 안전하게 변환
                                if not pd.api.types.is_string_dtype(combined_yfinance_df['date']):
                                    combined_yfinance_df['date'] = combined_yfinance_df['date'].astype(str)
                                
                                # stock_data 테이블 (yfinance 호환 스키마)에 저장 - DELETE/INSERT 방식
                                # 먼저 기존 데이터 삭제 (EXISTS 구문 사용)
                                conn.execute("""
                                    DELETE FROM stock_data 
                                    WHERE EXISTS (
                                        SELECT 1 FROM combined_yfinance_df 
                                        WHERE combined_yfinance_df.symbol = stock_data.symbol 
                                        AND DATE(combined_yfinance_df.date) = stock_data.date
                                    )
                                """)
                                ...
                                conn.execute("""
                                    ...
                                    SELECT symbol, DATE(date), open, high, low, close, volume, CURRENT_TIMESTAMP
                            ...
                            if raw_dfs:
                                ...
                                # 날짜 컬럼 타입 확인 및 변환
                                if not pd.api.types.is_string_dtype(combined_raw_df['date']):
                                    combined_raw_df['date'] = combined_raw_df['date'].astype(str)
                                
                                # stock_data_fdr_raw 테이블에 원본 데이터 저장 - DELETE/INSERT 방식
                                # 먼저 기존 데이터 삭제 (EXISTS 구문 사용)
                                conn.execute("""
                                    DELETE FROM stock_data_fdr_raw 
                                    WHERE EXISTS (
                                        SELECT 1 FROM combined_raw_df 
                                        WHERE combined_raw_df.symbol = stock_data_fdr_raw.symbol 
                                        AND DATE(combined_raw_df.date) = stock_data_fdr_raw.date
                                    )
                                """)
                                ...
                                conn.execute("""
                                    ...
                                    SELECT symbol, DATE(date), open, high, low, close, volume, adj_close, CURRENT_TIMESTAMP
                        ...
                        if "Resource temporarily unavailable" in str(e) and attempt < max_retries - 1:
                            wait_time = (attempt + 1) * 3  # 3, 6초 대기
                            self.logger.warning(f"배치 삽입 락 충돌, {wait_time}초 후 재시도 ({attempt + 1}/{max_retries})")
                            time.sleep(wait_time)
                            continue
                        else:
                            raise
    ...
    def collect_bulk_data(self, symbols: List[str], 
        ...
        self.logger.info(f"대량 데이터 수집 시작: {total_symbols}개 종목 (yfinance 호환)")
            ...
            for i in range(0, total_symbols, batch_symbols):
                ...
                with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                    ...
                    for future in as_completed(future_to_symbol):
                        ...
                        try:
                            result_dict = future.result(timeout=60)  # 60초 타임아웃
                            if result_dict is not None:
                                data_batch.append(result_dict)
                                processed_count += 1
                            else:
                                failed_count += 1
    ...
    def close(self):
        """연결 정리"""
        # 더 이상 _write_conn이 없으므로 정리할 것이 없음
        self.logger.info("BulkDataCollector 정리 완료")
